...
void Texture::Draw( const Rectf& dstRect, const Rectf& srcRect ) const
{
	...
	float vertexLeft{ dstRect.left - (defaultDestWidth * m_Pivot.x)};
	float vertexBottom{ dstRect.bottom - (defaultDestHeight * m_Pivot.y)};
	float vertexRight{};
	float vertexTop{};
...
void Texture::SetPivot(const Point2f& pivot)
{
	m_Pivot = pivot;
}

void Texture::SetPivot(float pivotX, float pivotY)
{
	SetPivot(Point2f{ pivotX, pivotY });
}
